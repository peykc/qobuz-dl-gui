...
*   **Synced Lyrics Sidecars**: Optional `.lrc` generation while downloading via the LRCLIB public API.
...
- **Synced Lyrics**: Enable `Synced Lyrics (.lrc)` to fetch lyrics and save them next to each track.
  - Lyrics are resolved via the public [LRCLIB](https://lrclib.net) API only (`/api/get` then `/api/search`).
